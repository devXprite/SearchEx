...
const fetchVideosResults = (q, page) => {
    ...
    return new Promise((resolve, reject) => {
        fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=50&q=${q}&type=video&order=viewCount&key=${process.env.YOUTUBE_API_KEY}`)
            .then(response => response.json())
            .then(responseJson => {
                const responseItems = responseJson.items;
                ...
                const items = responseItems.slice((page - 1) * 4, (page - 1) * 4 + 4).map((item, i) => ({
